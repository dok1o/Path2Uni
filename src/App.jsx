...
const XP_START = 1240
const XP_PROGRESS_KEY = 'path2uni:quest-progress'

function readQuestProgress() {
  try {
    const saved = JSON.parse(localStorage.getItem(XP_PROGRESS_KEY))
    return saved && typeof saved === 'object' ? saved : {}
  } catch { return {} }
}

function xpForQuest(task, index) {
  const count = task.subtasks?.length || 1
  const total = Number(task.xp) || 0
  return Math.floor(total / count) + (index < total % count ? 1 : 0)
}

function applyQuestProgress(plan, progress) {
  let previousStageDone = true
  return {
    ...plan,
    tasks: plan.tasks.map(task => {
      const completedSubtasks = [...new Set((progress[task.id] || []).filter(index => Number.isInteger(index) && index >= 0 && index < task.subtasks.length))].sort((a, b) => a - b)
      const isDone = completedSubtasks.length === task.subtasks.length
      const state = isDone ? 'done' : previousStageDone ? 'current' : 'locked'
      previousStageDone = isDone
      return { ...task, state, completedSubtasks, subtaskXp:task.subtasks.map((_, index) => xpForQuest(task, index)) }
    }),
  }
}

function calculateXp(plan, progress) {
  return plan.tasks.reduce((total, task) => total + (progress[task.id] || []).reduce((sum, index) => sum + xpForQuest(task, index), 0), XP_START)
}

function GamePath({ setChatOpen, plan, onOpenOSINT, xp, onCompleteQuest }) {
  const [selected, setSelected] = useState(1)
  const levels = plan.tasks.map((task, index) => ({ ...task, id:index + 1, side:index % 2 ? 'right' : 'left' }))
  const active = levels.find(level => level.id === selected) ?? levels[0]
  const completed = new Set(active.completedSubtasks || [])
  const doneStages = levels.filter(level => level.state === 'done').length
  return <main className="page game-page"><section className="game-head"><div><span className="eyebrow purple">MY PATH · AI-GENERATED</span><h1>Your next chapter<br/>starts here.</h1><p>Complete the quests to earn XP and unlock the next stage.</p><div className="ai-task-types">{['research','documents','application'].map(type => <span key={type}><i><TaskIcon type={type}/></i>{type}</span>)}</div></div><button className="game-leo-tip" onClick={() => setChatOpen(true)}><img src={mascot} alt="Leo mascot"/><span><b>Leo’s hint</b><small>Complete a grey quest to earn XP.</small></span><i>{icons.chevron}</i></button></section><section className="game-meta"><div><span className="game-stat-icon">✦</span><b>{xp.toLocaleString('en-US')}</b><small>XP earned</small></div><div><span className="game-stat-icon fire">♨</span><b>7 days</b><small>current streak</small></div><div><span className="game-stat-icon gem">◆</span><b>{levels.length}</b><small>AI tasks</small></div><div><span className="game-stat-icon energy">⚡</span><b>{doneStages} / {levels.length}</b><small>tasks done</small></div></section><section className="game-map-shell"><div className="game-map-title"><span>AI ROADMAP · CONNECTED TO OSINT</span><h2>Research to application</h2><small>Select an available point, then complete its quests.</small></div><div className="game-map"><svg className="game-line" viewBox="0 0 100 100" preserveAspectRatio="none"><path d="M29 17 C72 28 70 39 29 48 S28 68 70 80"/></svg>{levels.map((level,index) => <button key={level.id} disabled={level.state === 'locked'} style={{top:`${16 + index * 31}%`}} className={`game-level ${level.state} ${level.side} ${selected === level.id ? 'selected' : ''}`} onClick={() => setSelected(level.id)}><span className="level-disc"><i><TaskIcon type={level.type}/></i><b>{level.id}</b></span><span className="level-label"><strong>{level.shortTitle}</strong><small>{level.state === 'locked' ? 'Finish the previous stage' : level.state === 'done' ? 'Completed' : 'Ready to work'}</small></span></button>)}<div className="path-reward reward-one">✦<small>Earn XP</small></div><div className="path-reward reward-two">✉<small>AI task</small></div></div><aside className="level-panel"><span className="panel-kicker">AI TASK · {active.id} OF {levels.length}</span><div className={`panel-task-icon ${active.type}`}><TaskIcon type={active.type}/></div><h2>{active.title}</h2><p>{active.description}</p><div className="level-quests">{active.subtasks.map((task, index) => { const isDone = completed.has(index); return <button type="button" key={task} className={`quest ${isDone ? 'complete' : ''}`} disabled={active.state !== 'current' || isDone} onClick={() => onCompleteQuest(active.id, index)}><i>{isDone ? '✓' : index + 1}</i><span>{task}</span>{!isDone && <b>+{active.subtaskXp[index]} XP</b>}</button> })}</div><button className="button primary" onClick={() => onOpenOSINT(active.type)}>Open evidence graph <span>{icons.arrow}</span></button></aside></section></main>
}
...
function LiveDashboard({ setChatOpen, setPage, plan, xp, completedQuests }) {
  const doneStages = plan.tasks.filter(task => task.state === 'done').length
  return <main className="page dashboard-page"><Greeting setChatOpen={setChatOpen}/><section className="stats-row"><Stat icon="♨" value="7 days" label="Current streak" note="Your longest: 12 days" className="orange"/><Stat icon="✦" value={xp.toLocaleString('en-US')} label="Your XP" note={`${completedQuests} quests completed`} className="violet"/><Stat icon="◒" value={`${doneStages} / ${plan.tasks.length}`} label="Stages complete" note={doneStages === plan.tasks.length ? 'Your current path is complete' : 'Complete quests to unlock the next stage'} className="blue"/></section><section className="dash-grid"><HomeQuestPath setPage={setPage}/><aside className="side-stack"><article className="deadline-card"><div className="card-top"><span className="warning-dot">!</span><span>NEXT STEP</span><button>•••</button></div><h3>Keep earning XP</h3><p>Complete the available quest in My Path to light up your progress.</p><div className="deadline-bottom"><strong>{completedQuests} quests complete</strong><button className="round-arrow" onClick={() => setPage('roadmap')}>{icons.arrow}</button></div></article><article className="friend-card"><div className="card-top"><span>YOUR CREW</span><button className="text-button" onClick={() => setPage('friends')}>See all</button></div><div className="avatars"><span className="avatar a1">A</span><span className="avatar a2">L</span><span className="avatar a3">N</span><span className="avatar a4">+4</span></div><p><b>Amir</b> just completed “Choose your test”. Send a high-five!</p><button className="high-five">✋ Send a high-five</button></article></aside></section></main>
}
...
>>>>>>> origin/main
      ...
      <aside className="country-sidebar"><span className="country-badge"><CountryFlag country={country}/>DESTINATION MAP</span><h2>{city?.name || country.name}</h2><p>{city ? `Compare universities in ${city.name} and save the ones you want to revisit.` : 'Pick a glowing city point, or drag the country contour to see it from another angle.'}</p><div className="country-facts">{city ? <><div><b>{city.universities.length}</b><small>universities shown</small></div><div><b>{countryFavorites.length}</b><small>saved here</small></div></> : <><div><b>{country.cities.length}</b><small>cities mapped</small></div><div><b>{totalUniversities}</b><small>universities to explore</small></div></>}</div>{!city && <div className="map-legend"><span><i className="legend-pulse"/>Tap a glowing point</span><small>Every outline uses its national flag colours and can rotate in 3D.</small></div>}<button className="button primary" onClick={() => city ? setShortlistOpen(true) : selectCity(country.cities[0])}>{city ? `Show ${city.name} universities` : `Explore ${country.cities[0].name}`} <span>{icons.arrow}</span></button></aside>
    </section>
    <AnimatePresence>{shortlistOpen && city && <motion.div className="city-shortlist-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setShortlistOpen(false)}><motion.section className="city-shortlist-modal" initial={{opacity:0,y:42,scale:.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:30,scale:.98}} transition={{type:'spring',stiffness:220,damping:24}} onClick={event => event.stopPropagation()}><header><div><span className="eyebrow purple">CITY UNIVERSITY LIST</span><h2>{city.name}, {country.name}</h2><p>{city.universities.length} universities matching your destination.</p></div><button onClick={() => setShortlistOpen(false)} aria-label="Close city universities">×</button></header><div className="city-shortlist-list">{city.universities.map((university,index) => { const saved = Boolean(savedItem(university)); const people = friends.filter(friend => friend.university === university.name); return <motion.article key={university.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:index*.06}}><span className="shortlist-number">0{index + 1}</span><div><h3>{university.name}</h3><p>{university.match} · English programmes</p><span className="uni-social-proof"><span className="friend-stack">{people.slice(0,3).map(friend => <i key={friend.id} title={`${friend.name} ${friend.nickname}`} className={`friend-dot ${friend.className}`}>{friend.initials}</i>)}</span><em>{people.length ? `${people.length} from your crew chose this university` : 'No friends here yet'}</em></span></div><button className={`favorite-star ${saved ? 'saved' : ''}`} onClick={() => toggleUniversity(university)} aria-label={saved ? `Remove ${university.name} from saved` : `Save ${university.name}`}>{saved ? '★' : '☆'}</button></motion.article> })}</div><footer><small>Use the star to save universities to your profile.</small><button className="button soft" onClick={() => setShortlistOpen(false)}>Back to {city.name}</button></footer></motion.section></motion.div>}</AnimatePresence>
  </main>
}
...
export default function App() {
  ...
  const planWithProgress = useMemo(() => applyQuestProgress(admissionPlan, questProgress), [admissionPlan, questProgress])
  const earnedXp = useMemo(() => calculateXp(admissionPlan, questProgress), [admissionPlan, questProgress])
  const completedQuests = useMemo(() => Object.values(questProgress).reduce((total, items) => total + (Array.isArray(items) ? items.length : 0), 0), [questProgress])
  ...
  useEffect(() => { localStorage.setItem(XP_PROGRESS_KEY, JSON.stringify(questProgress)) }, [questProgress])
  ...
  const completeQuest = (taskNumber, subtaskIndex) => {
    const task = planWithProgress.tasks[taskNumber - 1]
    if (!task || task.state !== 'current' || task.completedSubtasks.includes(subtaskIndex)) return
    setQuestProgress(current => ({ ...current, [task.id]:[...(current[task.id] || []), subtaskIndex] }))
  }
