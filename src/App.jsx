  ...
  return <main className="page country-page country-library-page">
    ...
    <section className={`country-explorer country-explorer-3d ${city ? 'city-open' : ''}`}>
      <div className="country-stage country-stage-3d"><CountryMap3D country={country} city={city} onSelectCity={selectCity}/>
        <AnimatePresence mode="wait">{city && <motion.div key={`${country.id}-${city.name}`} className="city-scene visible social-city-scene" initial={{ opacity: 0, scale: .92, x: 45 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: .96, x: -30 }} transition={{ type:'spring',stiffness:170,damping:22 }}><CityBackdrop city={city.name}/><button className="city-back" onClick={() => setCity(null)}>← {t('Back to {country}', { country: t(country.name) })}</button><div className="city-title"><span className="eyebrow">{t('WELCOME TO')}</span><h2>{t(city.name)}</h2><p>{tParts(city.note)}</p></div><div className="city-universities">{city.universities.map((university,index)=>{const saved=Boolean(savedItem(university));const people=friends.filter(friend=>friend.university===university.name);return <motion.article layout key={university.id} className={`city-uni-card social-uni-card ${saved?'is-saved':''}`} whileHover={{y:-4}}><span className="uni-pin">{index+1}</span><span className="uni-card-main"><strong>{university.name}</strong><small>{tParts(university.focus)}</small><span className="uni-social-proof"><span className="friend-stack">{people.slice(0,3).map(friend=><i key={friend.id} title={`${friend.name} ${friend.nickname}`} className={`friend-dot ${friend.className}`}>{friend.initials}</i>)}</span><em>{people.length?`${people.slice(0,2).map(friend=>friend.name).join(' & ')}${people.length>2?` +${people.length-2}`:''} chose this university`:saved?t('Saved to your shortlist'):t('Explore programmes and admissions')}</em></span></span><motion.button whileTap={{scale:.78,rotate:-15}} className={`favorite-star ${saved?'saved':''}`} onClick={()=>toggleUniversity(university)} aria-label={saved?`Remove ${university.name} from saved`:`Save ${university.name}`}>{saved?'★':'☆'}</motion.button></motion.article>})}</div></motion.div>}</AnimatePresence>
      </div>
...
function ProfileV2({ favorites, setPage, onToggleFavorite, friends, highFives, onToggleHighFive, onLogout, user, profile, applicantProfile, onOpenExamStep, onEditProfile }) {
  ...
  const tabs = [{id:'overview',label:'Overview'}, {id:'security',label:'Account'}, {id:'saved',label:'Saved',count:favorites.length}, {id:'friends',label:'Friends',count:friends.length}]
